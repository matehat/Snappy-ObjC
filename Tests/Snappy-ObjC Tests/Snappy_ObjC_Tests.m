...
- (void)testCompressionIntegrity {
    ...
    NSData *compressedData = [randomData compressedData];
    XCTAssertEqualObjects([compressedData decompressedData], randomData, @"Decompressing the compressed data should yield the original data");
    
    randomData = randomNSData(20*1024*1024);
    compressedData = [randomData compressedData];
    XCTAssertEqualObjects([compressedData decompressedData], randomData, @"Decompressing the compressed data should yield the original data");
}
...
- (void)testTextCompression {
    ...
    NSData *compressed = [text compressedString];
    XCTAssert(text.length > compressed.length, @"Compressed text should be smaller than uncompressed");
    XCTAssertEqualObjects(text, [compressed decompressedString], @"Decompressing the compressed text should yield the original text");
}
...
- (void)testJSONCompression {
    ...
    NSData *compressed = [original compressedData];
